...
final class MainWindowViewModelTests: XCTestCase {
    ...
    func testOutputCollisionPreviewShowsVersionedFinalName() throws {
        let viewModel = makeViewModel(
            coordinator: RenderCoordinatorSpy(preparation: makePreparation()),
            preferencesStore: makePreferencesStore()
        )
        ...
        viewModel.outputDirectoryURL = directory
        viewModel.outputFilename = "Summer"
        try Data().write(to: directory.appendingPathComponent("Summer.mp4"))

        XCTAssertEqual(viewModel.currentRenderOutputNamePreview, "Summer")
        XCTAssertEqual(viewModel.currentRenderResolvedOutputNamePreview, "Summer-v2")
        XCTAssertEqual(
            viewModel.currentRenderOutputCollisionDescription,
            "A file with that name already exists. This render will save as Summer-v2."
        )
    }
    ...
    func testCancellingFullYearQueueScanDoesNotAddPartialJobs() async throws {
        let photoDiscovery = PhotoLibraryDiscoveringSpy(
            monthItems: [
                MonthYear(month: 1, year: 2025): [makeImageItem(id: "jan", captureDate: makeDate(year: 2025, month: 1, day: 2))]
            ],
            monthDiscoveryDelay: .milliseconds(250)
        )
        let viewModel = makeViewModel(
            coordinator: RenderCoordinatorSpy(preparation: makePreparation()),
            photoDiscovery: photoDiscovery,
        ...
        await waitUntil(message: "Timed out waiting for year scan to begin.") {
            viewModel.isPreparingYearQueue
        }
        XCTAssertTrue(viewModel.canCancelYearQueueScan)

        viewModel.cancelSelectedYearQueueScan()
        await waitUntil(message: "Timed out waiting for year scan cancellation.") {
            !viewModel.isPreparingYearQueue
        }

        XCTAssertTrue(viewModel.queuedRenderJobs.isEmpty)
        XCTAssertEqual(viewModel.statusMessage, "Year scan cancelled.")
    }

    func testStaleAlbumLoadDoesNotOverwriteSelectionAfterLeavingAlbumMode() async throws {
        let photoDiscovery = PhotoLibraryDiscoveringSpy(
            albums: [
                PhotoAlbumSummary(localIdentifier: "album-1", title: "Vacation", assetCount: 5)
            ],
            albumDiscoveryDelay: .milliseconds(150)
        )
        let viewModel = makeViewModel(
            coordinator: RenderCoordinatorSpy(preparation: makePreparation()),
            photoDiscovery: photoDiscovery,
        ...
        viewModel.sourceMode = .photos
        viewModel.selectedPhotosFilterMode = .album
        await waitUntil(message: "Timed out waiting for album load to begin.") {
            viewModel.isLoadingPhotoAlbums
        }
        ...
        XCTAssertEqual(viewModel.selectedPhotosFilterMode, .monthYear)
        XCTAssertTrue(viewModel.photoAlbums.isEmpty)
        XCTAssertEqual(viewModel.selectedPhotoAlbumID, "")
    }
...
private final class PhotoLibraryDiscoveringSpy: PhotoLibraryDiscovering, @unchecked Sendable {
    ...
    init(
        ...
        monthDiscoveryDelay: Duration? = nil,
        albumDiscoveryDelay: Duration? = nil
    ) {
        ...
        self.albums = albums
        self.monthDiscoveryDelay = monthDiscoveryDelay
        self.albumDiscoveryDelay = albumDiscoveryDelay
    }
    ...
    func discover(monthYear: MonthYear, timeZone: TimeZone) async throws -> [MediaItem] {
        if let monthDiscoveryDelay {
            try await Task.sleep(for: monthDiscoveryDelay)
        }
    ...
    func discoverAlbums() async throws -> [PhotoAlbumSummary] {
        if let albumDiscoveryDelay {
            try await Task.sleep(for: albumDiscoveryDelay)
        }
        return albums
    }
}
