...
final class MainWindowViewModel: ObservableObject {
    ...
    struct QueuedRenderSnapshot: Equatable, Sendable {
        ...
        let preserveResumableHDRCheckpoints: Bool
    }
    ...
    @Published var preserveResumableHDRCheckpoints: Bool = false {
        didSet { handleRenderSettingChange() }
    }
    ...
    var frameRateDescription: String {
        ...
        case .smart:
            return "Smart lets HDR HEVC use mixed cadence by default: stills render at 5 fps, title sections stay at 30 fps, and video sections follow standard source-fps buckets up to 60 fps."
        }
    }
    ...
    var currentRenderResolvedOutputFilenamePreview: String {
        resolvedOutputFilenamePreview(for: makeCurrentRenderSnapshot())
    }
    ...
    private func executeRenderSnapshot(
        ...
        let renderResult: RenderResult
        do {
            renderResult = try await renderSingleRequest(
                preparedSession: preparedSession,
                request: request,
                writeDiagnosticsLog: snapshot.writeDiagnosticsLog,
                progressMapper: progressMapper,
    ...
    private func makeCurrentRenderSnapshot() -> QueuedRenderSnapshot {
        QueuedRenderSnapshot(
            ...
            preserveResumableHDRCheckpoints: preserveResumableHDRCheckpoints
        )
    }
    ...
    private func resolvedOutputNamePreview(for snapshot: QueuedRenderSnapshot) -> String {
        resolvedOutputURLPreview(for: snapshot)
            .deletingPathExtension()
            .lastPathComponent
    }

    private func resolvedOutputFilenamePreview(for snapshot: QueuedRenderSnapshot) -> String {
        resolvedOutputURLPreview(for: snapshot).lastPathComponent
    }

    private func resolvedOutputURLPreview(for snapshot: QueuedRenderSnapshot) -> URL {
    ...
    private func makeSelectedYearQueueSnapshot(
        ...
        let seededSnapshot = QueuedRenderSnapshot(
            ...
            preserveResumableHDRCheckpoints: baseSnapshot.preserveResumableHDRCheckpoints
        )
        ...
        return QueuedRenderSnapshot(
            ...
            preserveResumableHDRCheckpoints: seededSnapshot.preserveResumableHDRCheckpoints
        )
    }
    ...
    private func applyPersistedRenderSettings() {
        ...
        preserveResumableHDRCheckpoints = settings.preserveResumableHDRCheckpoints ?? false
